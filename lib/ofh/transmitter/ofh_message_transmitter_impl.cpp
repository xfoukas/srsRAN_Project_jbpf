...
#ifdef JBPF_ENABLED
{
  for (unsigned i = prev_size; i < read_frames.size(); ++i) {
    auto& frame = read_frames[i];
    const auto frame_buf = frame->data();
    hook_capture_xran_packet(frame_buf.data(), frame_buf.size(), 0, JBPF_DL);
  }
}
#endif
