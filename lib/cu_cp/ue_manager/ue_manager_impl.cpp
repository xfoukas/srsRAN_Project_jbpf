                              ...
                              std::optional<du_cell_index_t> pcell_index)
{
...
#ifdef JBPF_ENABLED 
  {
    ...
    printf("MJB hook_cucp_uemgr_ue_add ue_index %ld plmn %s pci=%d rnti=%d \n", 
      (uint64_t)new_ue_index, 
      plmn.to_string().c_str(),
      pci.has_value()?pci.value():0 , 
      rnti.has_value() ? (uint16_t)to_value(rnti.value()) : 0);
...
void ue_manager::remove_ue(ue_index_t ue_index)
{
...
#ifdef JBPF_ENABLED 
  {
    ...
    printf("MJB hook_cucp_uemgr_ue_remove: ue_index %ld \n", (uint64_t)ue_index);
                                        ...
                                        du_cell_index_t pcell_index)
{
  ...
  {
    struct jbpf_cucp_uemgr_ctx_info ctx_info = {0, 0, 0, (uint64_t)ue_index};
    printf("MJB hook_cucp_uemgr_ue_update: ue_index %ld \n", (uint64_t)ue_index);
